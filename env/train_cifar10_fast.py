...
FAST VERSION - Optimized for quick validation
- Reduced epochs (20 instead of 100)
- Larger batch size (128 instead of 64)
- Fewer clients (10 instead of 20)
...
class ContractSFL:
    """Contract-based Split Federated Learning System - FAST VERSION"""
    
    def __init__(self, config, num_clients=10, epochs=20, lr=0.001, 
                 batch_size=128, device='cuda', output_dir='./env/outputs'):
        self.config = config
        ...
        print("="*80)
        print("Contract-based SFL - FAST VALIDATION MODE")
        print("="*80)
        print(f"Device: {self.device}")
        print(f"Number of clients: {num_clients} (reduced from 20)")
        print(f"Epochs: {epochs} (reduced from 100)")
        print(f"Batch size: {batch_size} (increased from 64)")
        print(f"Learning rate: {lr}")
        print(f"Expected time: ~30-40 minutes on RTX 2060 Super")
        ...
        self.train_loaders, self.test_loaders = get_client_dataloaders(
            ...
            batch_size=batch_size, num_workers=2  # Use 2 workers for faster loading
        )
    ...
    def save_checkpoint(self, epoch, elapsed_time):
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            # Federated averaging every 3 epochs (more frequent for faster convergence)
            if (epoch + 1) % 3 == 0:
            ...
            print(f"  Time: {epoch_time:.1f}s/epoch | Elapsed: {elapsed_time/60:.1f}min | ETA: {estimated_remaining/60:.1f}min")
            
            # Save checkpoint every 5 epochs
            if (epoch + 1) % 5 == 0:
                self.save_checkpoint(epoch + 1, elapsed_time)
    ...
    def save_results(self, elapsed_time):
        ...
        output_file = os.path.join(self.output_dir, f"训练输出数据_FAST_{timestamp}.txt")
        
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write("="*80 + "\n")
            f.write("Contract-based SFL - CIFAR-10 Training Results (FAST MODE)\n")
            f.write("="*80 + "\n\n")
            ...
            f.write("ResNet-18 Load Profile:\n")
            for v in range(1, 6):
    ...
    def plot_results(self, timestamp):
        ...
        plot_file = os.path.join(self.output_dir, f"training_curves_FAST_{timestamp}.png")
...
def main():
    ...
    num_clients = 10       # Reduced from 20
    epochs = 20            # Reduced from 100
    lr = 0.001
    batch_size = 128       # Increased from 64
    
    print("\n" + "="*80)
    print("FAST VALIDATION MODE")
    print("="*80)
    print("Optimizations:")
    print("  - Clients: 10 (vs 20)")
    print("  - Epochs: 20 (vs 100)")
    print("  - Batch size: 128 (vs 64)")
    print("  - FedAvg: every 3 epochs (vs 5)")
