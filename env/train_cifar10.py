...
Main training program using SplitResNet18 with dynamic split points
"""
import torch
...
class ContractSFL:
    ...
    def __init__(self, config, num_clients=20, epochs=100, lr=0.001, 
        ...
        print("="*80)
        print("Contract-based Split Federated Learning - Initialization")
        print("="*80)
        ...
        print("\nSolving optimal contract (known distribution)...")
        ...
        self.train_loaders, self.test_loaders = get_client_dataloaders(
            ...
            batch_size=batch_size, num_workers=0  # 0 for Windows compatibility
        )
        
        # Initialize models based on contract
        print("\nInitializing models based on contract...")
        self.models = []
        self.optimizers = []
        self.cut_points = []
    ...
    def train_one_epoch(self, epoch):
        ...
        for idx in range(self.num_clients):
            ...
            if idx % 5 == 0 or idx == self.num_clients - 1:
                print(f"  Client {idx:2d} (v={v}) | Loss: {avg_loss:.4f} | Acc: {avg_acc:.2f}%")
    ...
    def save_results(self, elapsed_time):
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write("="*80 + "\n")
            f.write("Contract-based Split Federated Learning - Training Results\n")
            f.write("="*80 + "\n\n")
            ...
            f.write("Optimal Contract (Known Distribution):\n")
            v_distribution = {}
            for idx in range(self.num_clients):
                v_star, r_star = self.optimal_menu[idx]
                v_distribution[v_star] = v_distribution.get(v_star, 0) + 1
                f.write(f"  Client {idx:2d}: v*={v_star}, R*={r_star:.2f}\n")
